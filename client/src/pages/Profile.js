...
const Profile = ({onUpdateUser}) => {
    ...
    const displayFollowing = currentUser.following_relationships.map(follower => {
        return (<li key={follower.id} style={{listStyle:'none'}}><UserCard user={follower.follower} onUpdateUser={onUpdateUser}/> </li>)
    }); 

    const displayFollowers = currentUser.follower_relationships.map(following => {
    ...
    <Container>
            ...
            <span style={{display:'flex', justifyContent:'space-evenly'}}>
                <Button onClick={(e) => setDisplay(e.target.value)} value="Following">Following: {currentUser.following_count}</Button>
                <Typography> | </Typography>
                <Button onClick={(e) => setDisplay(e.target.value)} value="Followers">Followers: {currentUser.follower_count}</Button>
            </span>
            <Grid item xs={8} gutter textAlign={'center'} >
                <Typography variant="h4" >{display}</Typography>
                <br></br>
                {display === "Followers"? displayFollowers : ''}
                {display === "Following"? displayFollowing : ''}
            </Grid>
    </Container>
    )
